...
import com.proxu.app.contracts.MainAdapterListener
import com.proxu.app.databinding.FragmentGroupServerBinding
import com.proxu.app.databinding.ItemQrcodeBinding
...
import com.proxu.app.helper.SimpleItemTouchHelperCallback
...
class GroupServerFragment : BaseFragment<FragmentGroupServerBinding>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mainViewModel.updateListAction.observe(viewLifecycleOwner) { index ->
            LogUtil.e("GroupServerFragment", "updateListAction triggered: index=$index, subscriptionId=${mainViewModel.subscriptionId}, subId=$subId, match=${mainViewModel.subscriptionId == subId}")
            if (mainViewModel.subscriptionId != subId) {
                LogUtil.e("GroupServerFragment", "updateListAction SKIPPED - subscriptionId mismatch")
                return@observe
            }
    ...
    private fun updateEmptyState() {
        ...
        LogUtil.e("GroupServerFragment", "updateEmptyState: serversCache.size=${mainViewModel.serversCache.size}, subId=$subId, isEmpty=$isEmpty, emptyState visibility=${binding.emptyStateLayout.visibility}, refresh visibility=${binding.refreshLayout.visibility}")
        binding.emptyStateLayout.visibility = if (isEmpty) View.VISIBLE else View.GONE
        binding.refreshLayout.visibility = if (isEmpty) View.GONE else View.VISIBLE
        LogUtil.e("GroupServerFragment", "updateEmptyState AFTER: emptyState visibility=${binding.emptyStateLayout.visibility}, refresh visibility=${binding.refreshLayout.visibility}")
    }

    private fun createProfile() {
        val token = ProxuAuthManager.getToken(ownerActivity)
        if (token.isNullOrBlank()) {
            ownerActivity.toast(R.string.auth_login_required)
            return
        }
        
        binding.btnCreateProfile.isEnabled = false
        binding.btnCreateProfile.text = getString(R.string.proxu_creating_profile)
        
        LogUtil.e("GroupServerFragment", "createProfile: starting...")
        
        ProxuProfileSync.createProfile(ownerActivity, token) { result ->
            LogUtil.e("GroupServerFragment", "createProfile callback: added=${result.added}, message=${result.message}")
            ownerActivity.lifecycleScope.launch(Dispatchers.Main) {
                binding.btnCreateProfile.isEnabled = true
                binding.btnCreateProfile.text = getString(R.string.proxu_create_profile)
                ...
                if (result.added > 0) {
                    ...
                    ownerActivity.toastError(R.string.proxu_profile_creation_failed)
                }
            }
        }
    }
