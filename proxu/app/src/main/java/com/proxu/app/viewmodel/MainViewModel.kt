package com.proxu.app.viewmodel
...
import com.proxu.app.ProxuApplication
import com.proxu.app.AppConfig
import com.proxu.app.R
import com.proxu.app.dto.GroupMapItem
import com.proxu.app.dto.entities.ServersCache
import com.proxu.app.dto.entities.SubscriptionCache
import com.proxu.app.dto.SubscriptionUpdateResult
import com.proxu.app.dto.TestServiceMessage
import com.proxu.app.extension.matchesPattern
...
import com.proxu.app.handler.SpeedtestManager
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    override fun onCleared() {
        getApplication<ProxuApplication>().unregisterReceiver(mMsgReceiver)
    ...
    fun reloadServerList() {
        ...
        updateCache()
        // Force LiveData to emit even if value hasn't changed
        // This is critical after login when profiles are synced in background
        val currentValue = updateListAction.value
        if (currentValue == -1) {
    ...
    fun exportAllServer(): Int {
        ...
        val ret = AngConfigManager.shareNonCustomConfigsToClipboard(
            getApplication<ProxuApplication>(),
            serverListCopy
        )
        return ret
    }
    ...
    fun initAssets(assets: AssetManager) {
        viewModelScope.launch(Dispatchers.Default) {
            SettingsManager.initAssets(getApplication<ProxuApplication>(), assets)
        }
    }
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_START_SUCCESS -> {
                    getApplication<ProxuApplication>().toastSuccess(R.string.toast_services_success)
                    isRunning.value = true
                }
                ...
                AppConfig.MSG_STATE_START_FAILURE -> {
                    ...
                    if (!errorMessage.isNullOrBlank()) {
                        getApplication<ProxuApplication>().toastError(errorMessage)
                    } else {
                        getApplication<ProxuApplication>().toastError(R.string.toast_services_failure)
                    }
                    isRunning.value = false
                }
                ...
                AppConfig.MSG_MEASURE_CONFIG_NOTIFY -> {
                    ...
                    updateTestResultAction.value =
                        getApplication<ProxuApplication>().getString(R.string.connection_runing_task_left, content)
                }
